...
type TicketReminderConsumer struct {
	logger  *zap.Logger
...
// **Constructor**
func NewTicketReminderConsumer(logger *zap.Logger, clients *clients.Clients, queries *models.Queries) *TicketReminderConsumer {
	return &TicketReminderConsumer{
		logger:  logger,
		clients: clients,
		queries: queries,
	}
...
// **Start Subscription**
func (tc *TicketReminderConsumer) StartTicketReminderConsumer() error {
	tc.logger.Info("Starting Ticket Reminder Consumer...")

	err := tc.clients.NATSClient.QueueSubscribe("CRM_TICKETS", "crm.tickets.reminder.*", "ticket_reminder_worker", func(msg *nats.Msg) {
		tc.logger.Info("Received Ticket Reminder Event!", zap.String("subject", msg.Subject), zap.String("data", string(msg.Data)))

		// Directly process the event (No need to check the reminder time)
		err := tc.processTicketReminder(msg)
		if err != nil {
			tc.logger.Error("Processing failed, will retry", zap.Error(err))
			msg.Nak() // Retry processing
			return
		}
	...
	if err != nil {
		tc.logger.Error("Failed to subscribe to ticket reminder", zap.Error(err))
		return err
	}
...
// **Process Event**
func (tc *TicketReminderConsumer) processTicketReminder(msg *nats.Msg) error {
	...
	tc.logger.Info("Successfully Processed Ticket Reminder Event!",
	...
	watchers, err := tc.queries.GetRecipientsEmailsAndRoles(ctx, event.ReminderDetails.Recipients)
	if err != nil {
		tc.logger.Error("failed to fetch watchers", zap.Error(err))
		return err
	}

	subject := fmt.Sprintf("[Reminder] Action Required for Ticket ID: %s", event.Ticket.TicketID)
	// message := fmt.Sprintf(
	// 	"Reminder: Ticket ID %s needs attention.\n\nStatus: %s\nPriority: %s\nAssigned To: %s.",
	// 	event.Ticket.TicketID, event.Ticket.Status, event.Ticket.Priority, event.Ticket.AssignedTo,
	// )
	message := event.ReminderDetails.Message
...
func (tc *TicketReminderConsumer) moveToDLQ(msg *nats.Msg, event data.TicketEvent, err error) {
	if err == nil {
